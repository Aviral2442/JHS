...
const VendorList: React.FC = () => {
  ...
  const handleVerify = async (vendorId: number) => {
    try {
      await axios.patch(`${baseURL}/api/vendor/update_vendor_verify_status/${vendorId}`);
  ...
  const handleBlock = async (vendorId: number) => {
    try {
      await axios.patch(`${baseURL}/api/vendor/update_vendor_block_status/${vendorId}`);
      fetchVendors(pagination.page);
    } catch (error) {
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {vendor.vendor_status !== 0 && (
                            <button
                              ...
                              className="flex h-8 w-8 items-center justify-center rounded-lg bg-green-100 text-green-600
                                hover:bg-green-200 dark:bg-green-900/30 dark:text-green-400 dark:hover:bg-green-900/50 transition-colors"
                            >
                          ...
                          {/* Block */}
                          {vendor.vendor_status !== 2 && (
                            <button
                              onClick={() => handleBlock(vendor.vendor_id)}
                              title="Block Vendor"
                              className="flex h-8 w-8 items-center justify-center rounded-lg bg-red-100 text-red-600
                                hover:bg-red-200 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50 transition-colors"
                            >
                              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
