...
const ENDPOINT = "/api/role-base-access-control/get_modules_list";
...
const ModuleList: React.FC = () => {
  ...
  const fetchModules = useCallback(async (page = 1) => {
    try {
      ...
      const data = res.data;
      setModules(data?.jsonData?.modules_list || []);
      setPagination(
        data?.pagination || { page: 1, limit: 10, total: 0, totalPages: 1 },
      );
    } catch (error) {
      console.error("Failed to fetch modules list:", error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleStatusToggle = async (
    ...
    try {
      await axios.patch(
        `${baseURL}/api/role-base-access-control/update_module_status/${moduleId}`,
        {
          status: newStatus,
        },
      );
    ...
    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
      <div className="p-5 lg:p-6 space-y-5">
        ...
        <DataTableFilters
          ...
          onAddNew={() => navigate("/admin/modules/add")}
          statusOptions={[
            ...
            <tbody>
              {loading ? (
                ...
                modules.map((moduleItem, idx) => {
                  ...
                  const isActive = moduleItem.status == 0;
                      ...
                      <td className="px-4 py-2">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            onClick={() =>
                              navigate(
                                `/admin/modules/edit/${moduleItem.module_id}`,
                              )
                            }
                            title="Edit Module"
