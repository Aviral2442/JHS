...
const categoryValidationSchema = Yup.object().shape({
  category_level3_level2_id: Yup.string().required("Parent category is required"),
  category_level3_name: Yup.string().required("Category name is required"),
...
const AddCLThree: React.FC = () => {
  const { catLvl3Id } = useParams<{ catLvl3Id: string }>();
  const isEditMode = !!catLvl3Id;
  ...
  const [form, setForm] = useState<CategoryForm>({
    category_level3_level2_id: "",
    category_level3_name: "",
    category_level3_img: "",
  });

  // Fetch category level two list for dropdown
  useEffect(() => {
    const fetchCategoryLevelTwoList = async () => {
      try {
        const res = await axios.get(
          `${baseURL}/api/category/get_category_level_two_list`,
        );
        const categories = res.data?.jsonData?.category_level_two_list || [];
        setLevelTwoCategories(categories);
        if (!isEditMode && categories.length > 0) {
          setForm((prev) => ({
            ...prev,
            category_level3_level2_id: categories[0].category_level2_id.toString(),
          }));
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchCategoryDetails = async () => {
      try {
        setFetching(true);
        const res = await axios.get(
          `${baseURL}/api/category/get_category_level_three/${catLvl3Id}`,
        );
        const category = res.data?.jsonData?.category_level_three_details;
        if (category) {
          setForm({
            category_level3_level2_id: category.category_level3_level2_id || "",
            category_level3_name: category.category_level3_name || "",
            category_level3_img: "",
          });
          if (category.category_level3_img) {
            setImagePreview(`${baseURL}${category.category_level3_img}`);
          }
        }
      } catch (error) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      setForm((prev) => ({ ...prev, category_level3_img: base64 }));
      setImagePreview(base64);
      if (errors.category_level3_img) {
        setErrors((prev) => {
          const next = { ...prev };
          delete next.category_level3_img;
          return next;
        });
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const validationData =
        isEditMode && !form.category_level3_img
          ? { ...form, category_level3_img: "existing" }
          : form;
    ...
    try {
      ...
      if (isEditMode) {
        res = await axios.put(
          `${baseURL}/api/category/update_category_level_three/${catLvl3Id}`,
          form,
        );
      } else {
        res = await axios.post(
          `${baseURL}/api/category/add_category_level_three`,
          form,
        );
      }
  ...
  return (
    <>
      <PageMeta
        title={
          isEditMode
            ? "Edit Category Level Three | Admin"
            : "Add Category Level Three | Admin"
        }
        description={
          isEditMode ? "Edit category" : "Add a new category level three"
        }
      />
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
        <div className="p-5 lg:p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white">
              {isEditMode ? "Edit Category Level Three" : "Add Category Level Three"}
            </h3>
            <button
              type="button"
              onClick={() => navigate("/admin/category-level-three")}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="flex gap-5">
              ...
              <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                Category Level Two <span className="text-red-500">*</span>
              </label>
              <select
                name="category_level3_level2_id"
                value={form.category_level3_level2_id}
              ...
              >
                ...
                {levelTwoCategories.map((category) => (
                  <option key={category.category_level2_id} value={category.category_level2_id}>
                    {category.category_level2_name}
                  </option>
                ))}
              </select>
              {errors.category_level3_level2_id && (
            ...
            <div className="w-full">
              ...
              <input
                type="text"
                name="category_level3_name"
                value={form.category_level3_name}
                onChange={handleChange}
                placeholder="Enter category name"
                className="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-700 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:text-gray-300"
              />
              {errors.category_level3_name && (
                <p className="mt-1 text-xs text-red-500">
                  {errors.category_level3_name}
                </p>
              )}
