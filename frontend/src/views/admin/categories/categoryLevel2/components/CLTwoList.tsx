...
interface CategoryLevelTwoItem {
  ...
  category_level2_status: number;
  category_level2_createdAt: string;
}
...
const CLTwoList: React.FC = () => {
  const navigate = useNavigate();
  const [categories, setCategories] = useState<CategoryLevelTwoItem[]>([]);
  ...
  const canAdd = isAllowed("add");
  const canEdit = isAllowed("edit");
  const canExport = isAllowed("export");
  const canActive = isAllowed("active");
  const canInactive = isAllowed("inactive");
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
      <div className="p-5 lg:p-6 space-y-5">
        <DataTableFilters
          title="Category Level Two List"
          onFilterChange={handleFilterChange}
          onAddNew={canAdd ? () => navigate("/admin/category/level-two/add") : undefined}
        />

        <div className="flex flex-wrap items-center justify-between gap-3">
          {canExport && (
            <DatatableActionButton endpoint={ENDPOINT} dataAccess="category_level_two_list" />
          )}
            ...
            <tbody>
              {loading ? (
                ...
                categories.map((category, idx) => {
                  ...
                  const isActive = category.category_level2_status == 0;

                  return (
                    <tr
                      key={category.category_level2_id}
                    ...
                    >
                      ...
                      <td className="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                        {category.category_level2_id}
                      </td>

                      <td className="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                        {category.category_level2_img ? (
                          <img
                            src={`${baseURL}${category.category_level2_img}`}
                            alt={category.category_level2_name}
                      ...
                      <td className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        {formatDate(category.category_level2_createdAt)}
                      </td>
                      ...
                      <td className="px-4 py-2">
                        <div className="flex items-center justify-center gap-2">
                          {((isActive && canInactive) ||
                            (!isActive && canActive)) && (
                            <button
                              onClick={() => handleStatusToggle(category.category_level2_id, category.category_level2_status)}
                          ...
                          {canEdit && (
                            <button
                              onClick={() => navigate(`/admin/category/level-two/edit/${category.category_level2_id}`)}
                              title="Edit Category Level Two"
