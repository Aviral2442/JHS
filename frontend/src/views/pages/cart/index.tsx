...
const CartItemCard: React.FC<{
  ...
  return (
    <div className={`card-ui rounded-xl shadow-md p-4 mb-4 transition-all duration-300 ${isRemoving ? 'opacity-0 scale-95' : 'opacity-100'}`}>
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="shrink-0">
          <div className="relative w-full sm:w-32 h-32 rounded-lg overflow-hidden">
            ...
            />
            {!item.inStock && (
              <div className="absolute top-2 left-2 text-white text-xs px-2 py-1 rounded" style={{ backgroundColor: 'var(--gray-color)' }}>
                Out of Stock
              </div>
            )}
            {item.originalPrice && (
              <div className="absolute top-2 right-2 text-white text-xs px-2 py-1 rounded" style={{ backgroundColor: 'var(--sky-blue)' }}>
        ...
        <div className="grow">
          <div className="flex justify-between">
            <div>
              <h3 className="font-semibold text-lg" style={{ color: 'var(--black-color)' }}>{item.name}</h3>
              <p className="text-sm mt-1" style={{ color: 'var(--gray-color)' }}>{item.description}</p>
              <div className="flex items-center gap-2 mt-2">
                <span className="text-xs px-2 py-1 rounded" style={{ backgroundColor: 'var(--background-alt)', color: 'var(--black-color)' }}>
                  {item.category}
                </span>
                {item.inStock ? (
                  <span className="flex items-center text-xs" style={{ color: 'var(--sky-blue)' }}>
                    <Check size={12} className="mr-1" /> In Stock
                  </span>
                ) : (
                  <span className="flex items-center text-xs" style={{ color: 'var(--gray-color)' }}>
            ...
            <div className="hidden md:block text-right">
              ...
              {item.originalPrice && (
                <div className="line-through" style={{ color: 'var(--gray-color)' }}>
          ...
          <div className="md:hidden mt-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                {item.originalPrice && (
                  <div className="text-sm line-through" style={{ color: 'var(--gray-color)' }}>
                    ${item.originalPrice.toFixed(2)}
                  </div>
                )}
              </div>
              <div className="text-lg font-bold" style={{ color: 'var(--black-color)' }}>
          ...
          <div className="flex justify-between items-center mt-6">
            <div className="flex items-center space-x-4">
              {/* Quantity Selector */}
              <div className="flex items-center border rounded-lg" style={{ borderColor: 'var(--gray-color)' }}>
                <button
                  onClick={decrement}
                  className="p-2 hover:opacity-70 transition-colors disabled:opacity-50"
              ...
              <button
                ...
                style={{ 
                  color: item.isWishlisted ? 'var(--sky-blue)' : 'var(--gray-color)',
                  backgroundColor: item.isWishlisted ? 'var(--background-alt)' : 'transparent'
                }}
              >
            ...
            <button
              ...
              className="flex items-center hover:opacity-70 px-3 py-2 rounded-lg transition-colors"
...
const PromoCodeInput: React.FC<{
  ...
  return (
    <div className="card-ui rounded-xl shadow-md p-6">
      <h3 className="text-lg font-semibold mb-4" style={{ color: 'var(--black-color)' }}>Promo Code</h3>

      {appliedCode ? (
        <div className="flex items-center justify-between rounded-lg p-4" style={{ backgroundColor: 'var(--background-alt)', border: '1px solid var(--sky-blue)' }}>
          <div>
            <div className="flex items-center" style={{ color: 'var(--sky-blue)' }}>
              <Check size={18} className="mr-2" />
              <span className="font-medium">Code applied: {appliedCode.code}</span>
            </div>
            <p className="text-sm mt-1" style={{ color: 'var(--sky-blue)' }}>
        ...
        <>
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="grow px-4 py-3 border rounded-lg focus:outline-none focus:ring-2"
              style={{ borderColor: 'var(--gray-color)' }}
            />
            <button
              onClick={handleApply}
              className="btn-primary px-6 py-3 font-medium rounded-lg transition-all transform hover:-translate-y-0.5 shadow-md"
            >
              Apply
            </button>
          </div>
          {error && <p className="text-sm mt-2" style={{ color: 'var(--gray-color)' }}>{error}</p>}

          <div className="mt-4">
            <p className="text-sm mb-2" style={{ color: 'var(--gray-color)' }}>Available promo codes:</p>
            <div className="flex flex-wrap gap-2">
              {promoCodes.map(promo => (
                <span
                  key={promo.code}
                  className="text-xs px-3 py-1 rounded-full border"
                  style={{ backgroundColor: 'var(--background-alt)', color: 'var(--black-color)', borderColor: 'var(--gray-color)' }}
                >
                  {promo.code}
                </span>
              ))}
  ...
  return (
    <div className="card-ui rounded-xl shadow-md p-6 sticky top-6">
      ...
      <div className="space-y-4">
        <div className="flex justify-between">
          <span style={{ color: 'var(--gray-color)' }}>Items ({itemCount})</span>
        ...
        <div className="pt-4 mt-4 border-t" style={{ borderColor: 'var(--background-alt)' }}>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--gray-color)' }}>Including ${tax.toFixed(2)} in taxes</p>
        </div>
      </div>

      <button
        onClick={onCheckout}
        className="btn-primary w-full mt-8 py-4 font-bold rounded-xl transition-all transform hover:-translate-y-1 shadow-lg hover:shadow-xl flex items-center justify-center"
      >
...
const index: React.FC = () => {
  ...
  return (
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8" style={{ background: 'linear-gradient(to bottom right, var(--background-alt), var(--white-color))' }}>
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <header className="mb-10">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold" style={{ color: 'var(--black-color)' }}>Your Shopping Cart</h1>
              <p className="mt-2" style={{ color: 'var(--gray-color)' }}>Review your items and proceed to checkout</p>
            </div>
            <div className="flex items-center">
              <ShoppingBag size={28} className="mr-2" style={{ color: 'var(--sky-blue)' }} />
              <span className="text-xl font-bold" style={{ color: 'var(--black-color)' }}>{cartItems.length} items</span>
            </div>
          </div>
        ...
        {/* Main Content */}
        {cartItems.length === 0 ? (
          <div className="card-ui rounded-2xl shadow-md p-12 text-center">
            <ShoppingBag size={80} className="mx-auto mb-6" style={{ color: 'var(--gray-color)' }} />
            <h3 className="text-2xl font-bold mb-4" style={{ color: 'var(--black-color)' }}>Your cart is empty</h3>
            <p className="mb-8 max-w-md mx-auto" style={{ color: 'var(--gray-color)' }}>
              Looks like you haven't added any items to your cart yet. Start shopping to fill it up!
            </p>
            <button
              onClick={() => setCartItems(initialCartItems)}
              className="btn-primary px-8 py-3 font-medium rounded-lg transition-all transform hover:-translate-y-0.5 shadow-md flex items-center mx-auto"
            >
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-2/3">
              ...
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold" style={{ color: 'var(--black-color)' }}>
                  Cart Items <span style={{ color: 'var(--sky-blue)' }}>({cartItems.length})</span>
                </h2>
                {cartItems.length > 0 && (
                  <button
                    onClick={clearCart}
                    className="hover:opacity-70 px-4 py-2 rounded-lg transition-colors flex items-center"
                    style={{ color: 'var(--gray-color)' }}
                  >
                    <Trash2 size={18} className="mr-2" />
                    Clear Cart
            ...
            <div className="lg:w-1/3">
              ...
              <div className="mt-8 grid grid-cols-3 gap-4">
                <div className="card-ui p-4 rounded-xl text-center shadow-sm">
                  <div className="text-2xl font-bold" style={{ color: 'var(--sky-blue)' }}>100%</div>
                ...
                <div className="card-ui p-4 rounded-xl text-center shadow-sm">
                  ...
                  <div className="text-xs mt-1" style={{ color: 'var(--gray-color)' }}>Encrypted</div>
                </div>
                <div className="card-ui p-4 rounded-xl text-center shadow-sm">
                  <div className="text-2xl font-bold" style={{ color: 'var(--sky-blue)' }}>30</div>
                  <div className="text-xs mt-1" style={{ color: 'var(--gray-color)' }}>Day Returns</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {isLoading && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="p-8 rounded-2xl shadow-2xl text-center" style={{ backgroundColor: 'var(--white-color)' }}>
              ...
              <h3 className="text-xl font-bold" style={{ color: 'var(--black-color)' }}>Processing Order</h3>
              <p className="mt-2" style={{ color: 'var(--gray-color)' }}>Please wait while we complete your purchase...</p>
            </div>
          </div>
        )}
